...
export const PublicPayPage = () => {
  ...
  const handleProceedToPay = async () => {
    if (selectedMethod === "card") {
      ...
      try {
        // Call the secure Supabase Edge Function we just deployed!
        const { data, error } = await supabase.functions.invoke('vult-checkout', {
          body: {
            orderId: invoice.id,
        ...
        // Redirect the user instantly to the secure Vult checkout page
        if (data?.checkoutUrl) {
          window.location.href = data.checkoutUrl;
        } else {
          throw new Error("No checkout link returned from gateway.");
        }
      } catch (err: any) {
        console.error("Payment Initiation Error:", err);
        alert("Failed to connect to secure gateway: " + err.message);
      } finally {
        setIsProcessing(false);
      }
      
    } else {
      alert("Mobile Money flow coming soon!");
    }
  };
